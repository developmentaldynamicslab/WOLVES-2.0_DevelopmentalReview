...
elseif DCCS == 2
    historyDuration = floor((4000)/scale_factor);%Simulation time training; + a gap of 1 sec between every two trials%
    historyDuration2 = floor((4000)/scale_factor); %Simulation time test trials + a gap of 1 sec between every two trials
elseif DCCS == 3
...
sigma_exc = 5;
sigma_inh = 10;
vstrength = 5.25;% 5.25; % visual stimulus strength
wstrength = 8;% word stimulus strength -- is reset in DCCS code later

%%added more spatial positions for DCCS, so space is shifted in XSit
if DCCS > 0
    spt_L = 8;%10; %spatial locations for stimuli
    spt_Lm = 25;%30;
    spt_Lc=42;
    spt_C = 50;
    spt_Rc= 59;
    ...
    %code to use WOLVES XSit values...if 0, use DCCS values above...
...
for i = 1 : nFeatures
    ...
    %% new added for DCCS%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % sum across word dim, project across into wm_c field
    sim.addElement(GaussKernel1D(['wf' n ' -> wm_c' n], fieldSize_ftr, sigma_exc, 0, true, true, kcf), ...
        ['sum wf' n], 'horizontalSum');
    sim.addElement(ExpandDimension2D(['expand wf' n ' -> wm_c' n], 2, [fieldSize_ftr, fieldSize_spt]), ...
        ['wf' n ' -> wm_c' n], [], ['wm_c' n]);

    sim.addElement(GaussKernel1D(['wm_c' n ' -> wf' n], fieldSize_ftr, sigma_exc, 0, true, true, kcf), ...
        ['sum wm_c' n], 'horizontalSum');
    sim.addElement(ExpandDimension2D(['expand wm_c' n ' -> wf' n], 2, [fieldSize_ftr, fieldSize_wd]), ...
        ['wm_c' n ' -> wf' n], [], ['wf' n]);
...
%%This was commented out in DCCS -- ok to have it here?? YES--looks like it
%%is updated later in the DCCS code...

if DCCS > 0
    Property(1,:) = [30, 80, 130, 180, 230, 280];%%Shapes  %%more than or equal to nObjects
    Property(2,:) = [30, 80, 130, 180, 230, 280];%%Colors   %%more than or equal to nObjects
else
...
%%added more spatial positions for DCCS...
for i= 1:nFeatures  
    n =num2str(i);
    ...
    sim.addElement(GaussStimulus2D( strcat('Feature_',n,'_RightMid'), [fieldSize_ftr, fieldSize_spt],... 
        sigma_exc, sigma_exc, 0, 0, 0, true, true, false), ...
        [], [], ['vis_f' n]);
    sim.addElement(GaussStimulus2D( strcat('Feature_',n,'_LeftCentre'), [fieldSize_ftr, fieldSize_spt],... 
        sigma_exc, sigma_exc, 0, 0, 0, true, true, false), ...
        [], [], ['vis_f' n]);
    sim.addElement(GaussStimulus2D( strcat('Feature_',n,'_RightCentre'), [fieldSize_ftr, fieldSize_spt],...
        sigma_exc, sigma_exc, 0, 0, 0, true, true, false), ...
        [], [], ['vis_f' n]);
...
%%only 4 in original XSit -- so more words now
%%Write this in a loop as with the 
for i=1:wordcount
    sim.addElement(GaussStimulus1D(['Word_Label_' num2str(i)], fieldSize_wd, 0, 0, 0, true), [], [], 'word');
